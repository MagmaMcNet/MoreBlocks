...
public class MoreblocksModVariables {
	public static class MapVariables extends WorldSavedData {
		public static final String DATA_NAME = "moreblocks_mapvars";
		public boolean bread_Foxes = false;
		public boolean Spawn_Foxes = false;
		public boolean Foxes_Normal_Spawn = false;
		public boolean tmp_Bread_foxes = false;
		public String bread_foxes = "";
		...
		public void readFromNBT(NBTTagCompound nbt) {
			bread_Foxes = nbt.getBoolean("bread_Foxes");
			Spawn_Foxes = nbt.getBoolean("Spawn_Foxes");
			Foxes_Normal_Spawn = nbt.getBoolean("Foxes_Normal_Spawn");
			tmp_Bread_foxes = nbt.getBoolean("tmp_Bread_foxes");
			bread_foxes = nbt.getString("bread_foxes");
			PRINT_LOG = nbt.getBoolean("PRINT_LOG");
		}

		@Override
		public NBTTagCompound writeToNBT(NBTTagCompound nbt) {
			nbt.setBoolean("bread_Foxes", bread_Foxes);
			nbt.setBoolean("Spawn_Foxes", Spawn_Foxes);
			nbt.setBoolean("Foxes_Normal_Spawn", Foxes_Normal_Spawn);
			nbt.setBoolean("tmp_Bread_foxes", tmp_Bread_foxes);
			nbt.setString("bread_foxes", bread_foxes);
			nbt.setBoolean("PRINT_LOG", PRINT_LOG);
			return nbt;
		}
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				MoreblocksMod.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(0, this));
			} else {
				MoreblocksMod.PACKET_HANDLER.sendToAll(new WorldSavedDataSyncMessage(0, this));
			}
		}
	...
	public static class WorldVariables extends WorldSavedData {
		public static final String DATA_NAME = "moreblocks_worldvars";
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				MoreblocksMod.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(1, this));
			} else {
				MoreblocksMod.PACKET_HANDLER.sendToDimension(new WorldSavedDataSyncMessage(1, this), world.provider.getDimension());
			}
		}
	...
	public static class WorldSavedDataSyncMessageHandler implements IMessageHandler<WorldSavedDataSyncMessage, IMessage> {
		...
		private void syncData(WorldSavedDataSyncMessage message, MessageContext context, World world) {
			if (context.side == Side.SERVER) {
				...
				if (message.type == 0)
					MoreblocksMod.PACKET_HANDLER.sendToAll(message);
				else
					MoreblocksMod.PACKET_HANDLER.sendToDimension(message, world.provider.getDimension());
			}
			if (message.type == 0) {
